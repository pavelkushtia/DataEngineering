...
def generate_sample_data(spark: SparkSession, num_records: int = 10000) -> DataFrame:
    ...
    for i in range(num_records):
        record = {
            ...
            - timedelta(seconds=random.randint(0, 7 * 24 * 3600)),  # Last 7 days
...
def parse_common_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--app-name", default="SparkBuildingBlock", help="Spark application name"
    )

    parser.add_argument(
        "--executor-memory", default="2g", help="Executor memory (default: 2g)"
    )

    parser.add_argument(
        "--executor-cores", default="2", help="Executor cores (default: 2)"
    )
...
def print_dataframe_summary(
    df: DataFrame, name: str = "DataFrame", show_data: bool = True
):
    """
