...
try:
    ...
    from pyspark.sql.functions import (
        ...
        regexp_replace, upper, lower, trim,
        rank, dense_rank, row_number, lag, lead,
        collect_list, concat_ws, split, size
    )
...
def perform_aggregations(df: DataFrame) -> dict:
    ...
    customer_segments = user_behavior \
        .withColumn(
            "segment",
...
def perform_window_operations(df: DataFrame) -> DataFrame:
    ...
    windowed_df = df \
        .withColumn("user_order_number", row_number().over(user_window)) \
        ...
        .withColumn(
            "purchase_growth",
            when(col("previous_purchase").isNotNull(),
                 spark_round((col("total_amount") - col("previous_purchase")) / col("previous_purchase") * 100, 2)
            ).otherwise(0)
        )
...
def data_quality_checks(df: DataFrame) -> dict:
    ...
    quality_metrics["duplicates"] = {
        ...
        "duplicate_percentage": round(duplicate_count / total_records * 100, 2) if total_records > 0 else 0
    }
    ...
    validations = {
        ...
        "valid_categories": df.filter(~col("category").isin(["ELECTRONICS", "CLOTHING", "BOOKS", "HOME", "SPORTS"])).count(),
        "future_dates": df.filter(col("timestamp") > datetime.now()).count(),
        "reasonable_prices": df.filter((col("price") < 1) | (col("price") > 10000)).count()
    }
    ...
    distributions["categories"] = [{"category": row["category"], "count": row["count"]} for row in category_dist]
    
    # Action distribution  
    action_dist = df.groupBy("action").count().orderBy(desc("count")).collect()
    distributions["actions"] = [{"action": row["action"], "count": row["count"]} for row in action_dist]
...
@handle_errors
def save_results(results: dict, windowed_df: DataFrame, quality_metrics: dict, output_path: str):
    """
    ...
    try:
        ...
        results["category_revenue"].coalesce(1).write.mode("overwrite").csv(f"{output_path}/category_revenue", header=True)
        results["daily_trends"].coalesce(1).write.mode("overwrite").csv(f"{output_path}/daily_trends", header=True)
        results["hourly_patterns"].coalesce(1).write.mode("overwrite").csv(f"{output_path}/hourly_patterns", header=True)
        results["top_customers"].coalesce(1).write.mode("overwrite").csv(f"{output_path}/top_customers", header=True)
        results["customer_segments"].coalesce(1).write.mode("overwrite").csv(f"{output_path}/customer_segments", header=True)
        
        # Save windowed data (sample)
        print("   Saving windowed data sample...")
        windowed_df.sample(0.1).coalesce(1).write.mode("overwrite").parquet(f"{output_path}/windowed_sample")
        
        # Save full dataset in Parquet format (partitioned for better performance)
        print("   Saving full dataset...")
        windowed_df.write.mode("overwrite").partitionBy("category", "date").parquet(f"{output_path}/full_dataset")
        ...
        print(f"   - CSV files: category_revenue, daily_trends, hourly_patterns, top_customers, customer_segments")
...
def print_results_summary(results: dict, quality_metrics: dict):
    ...
    for i, row in enumerate(results["category_revenue"].take(3), 1):
        print(f"   {i}. {row['category']}: ${row['revenue']:,.2f} ({row['order_count']:,} orders)")
    
    # Customer segments
    print(f"\n👥 CUSTOMER SEGMENTS:")
    for row in results["customer_segments"].collect():
        print(f"   {row['segment']}: {row['customer_count']:,} customers, Avg: ${row['avg_spent']:.2f}")
    ...
    print(f"   Business Rule Violations: {business_issues:,} ({business_issues/total_records*100:.1f}%)")
    
    # Top null columns
    null_cols = [(col, data["null_percentage"]) for col, data in quality_metrics["null_analysis"].items() 
...
def main():
    ...
    try:
        ...
        print_dataframe_summary(cleaned_df, "Cleaned Dataset", show_data=not args.records > 10000)
