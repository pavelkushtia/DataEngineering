...
class ETLPipeline:
    ...
    def extract_data_sources(self, num_records: int) -> Dict[str, DataFrame]:
        ...
        for i in range(num_records // 5):
            analytics = Row(
                ...
                referrer=["google.com", "facebook.com", "direct", "email", "other"][i % 5],
    ...
    def transform_data(self, sources: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
        ...
        sales_transformed = sources["sales"] \
        ...
        customers_transformed = sources["customers"] \
            .withColumn("full_name_clean", 
                       concat(upper(substring(col("full_name"), 1, 1)),
            ...
            .withColumn("activity_status",
                       ...
                       .otherwise("Dormant")) \
            .withColumn("lifetime_value_tier",
        ...
        products_transformed = sources["products"] \
            .withColumn("markup_percentage", 
                       round((col("retail_price") - col("cost_price")) / col("cost_price") * 100, 2)) \
        ...
        web_transformed = sources["web_analytics"] \
            ...
            .withColumn("page_category", 
                       regexp_extract(col("page_url"), "/([^/]+)/", 1)) \
            .withColumn("is_weekend",
                       dayofmonth(col("timestamp")) % 7 < 2)
        ...
        # Customer summary
        customer_summary = sales_transformed \
            .groupBy("customer_id") \
            .agg(
    ...
    def data_quality_checks(self, transformed: Dict[str, DataFrame]) -> Dict[str, Any]:
        ...
        for table_name, df in transformed.items():
            ...
            for column in df.columns:
                ...
                table_quality["null_checks"][column] = {
                    ...
                    "null_percentage": round(null_count / table_quality["total_records"] * 100, 2)
                        if table_quality["total_records"] > 0 else 0
                }
            ...
            if table_quality["total_records"] > 0:
                ...
                table_quality["duplicates"] = {
                    ...
                    "duplicate_percentage": round(duplicate_count / table_quality["total_records"] * 100, 2)
                }
            
            # Table-specific business rules
            if table_name == "sales_fact":
                # Sales fact table validations
                table_quality["business_rules"]["negative_amounts"] = df.filter(col("final_amount") < 0).count()
                table_quality["business_rules"]["zero_quantities"] = df.filter(col("quantity") <= 0).count()
                table_quality["business_rules"]["future_dates"] = df.filter(col("timestamp") > current_timestamp()).count()
                table_quality["business_rules"]["invalid_discounts"] = df.filter(col("discount_amount") < 0).count()
                
            elif table_name == "customer_dim":
                # Customer dimension validations
                table_quality["business_rules"]["invalid_emails"] = df.filter(~col("email").contains("@")).count()
                table_quality["business_rules"]["future_registrations"] = df.filter(col("registration_date") > current_date()).count()
                table_quality["business_rules"]["negative_lifetime_value"] = df.filter(col("lifetime_value") < 0).count()
                
            elif table_name == "product_dim":
                # Product dimension validations
                table_quality["business_rules"]["negative_prices"] = df.filter(col("retail_price") <= 0).count()
                table_quality["business_rules"]["negative_stock"] = df.filter(col("stock_quantity") < 0).count()
                table_quality["business_rules"]["cost_higher_than_retail"] = df.filter(col("cost_price") > col("retail_price")).count()
        ...
        overall_quality_score = max(0, 100 - (total_issues / total_records * 100)) if total_records > 0 else 100
    ...
    def load_data(self, transformed: Dict[str, DataFrame], output_path: str):
        ...
        try:
            ...
            web_fact.write \
                ...
                .parquet(f"{output_path}/fact/web_analytics")
            ...
            customer_dim.write \
                .mode("overwrite") \
                .parquet(f"{output_path}/dim/customers")
            ...
            product_dim.write \
                .mode("overwrite") \
                .parquet(f"{output_path}/dim/products")
            ...
            daily_sales.write \
                .mode("overwrite") \
                .parquet(f"{output_path}/agg/daily_sales")
            ...
            customer_summary.write \
                .mode("overwrite") \
                .parquet(f"{output_path}/agg/customer_summary")
            ...
            business_summary = daily_sales.groupBy("category") \
                .agg(
            ...
            business_summary.coalesce(1).write \
                .mode("overwrite") \
                .csv(f"{output_path}/export/business_summary", header=True)
            
            # Top customers export
            top_customers = customer_summary.orderBy(col("total_spent").desc()).limit(100)
            top_customers.coalesce(1).write \
                .mode("overwrite") \
                .csv(f"{output_path}/export/top_customers", header=True)
    ...
    @handle_errors
    def generate_pipeline_report(self, quality_results: Dict[str, Any], output_path: str):
        """
        ...
        report = {
            ...
            "performance_metrics": {
                ...
                "records_per_second": sum(self.metrics["extraction"].values()) / total_duration,
...
def main():
    ...
    try:
        ...
        print(f"   Generated {sum(pipeline.metrics['loading'].values()):,} output records")
